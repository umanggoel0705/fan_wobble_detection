...
def home():
    ...
    if request.method == "POST":
        ...
        final_script.main(video_path)

        # Render the same home page but indicate that plots are ready
        print("FPS being passed to template:", final_script.FPS)
        return render_template("home.html", output="Tracking..", plots_ready=True, fps=final_script.FPS)
...
def results():
    ...
    return render_template("results.html", analysis=final_script.analysis_data)

@app.route("/video_feed")
def video_feed():
    if not video_path_global:
        return "No video uploaded yet.", 400
    return Response(final_script.tracker(video_path_global, final_script.ROI),
...
def video_feed_roi():
    ...
    print("ROI is being passed to template:", final_script.ROI)
    return Response(final_script.tracker_roi(video_path_global, final_script.ROI),
                    mimetype='multipart/x-mixed-replace; boundary=frame')
